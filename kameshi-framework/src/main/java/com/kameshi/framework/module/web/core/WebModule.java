package com.kameshi.framework.module.web.core;
...
import com.kameshi.framework.module.web.handler.IAfterRequestHandler;
...
public class WebModule extends KernelModule implements IWebModule {	
	...
	public void processRequest(ServletRequest request, ServletResponse response, FilterChain chain) {
		...
		if (request instanceof HttpServletRequest) {
			...
			LOG.warn("This web module implementation works only for http requests");
		}
	}
		...
		if (afterRequestHandlers != null && !afterRequestHandlers.isEmpty()) {
			for (IAfterRequestHandler handler : afterRequestHandlers) {
				try {
					...
					LOG.error("Error after the exception barrier " + exception.getMessage());
				}
			}
		}
	}
	...
	private void invokeApplication(FilterChain chain, ServletRequest request, ServletResponse response) throws IOException, ServletException {
		if (!getFlowManager().isRequestCommitted()) {
			...
			LOG.warn("Response had been commited before application was invoked...!");
		}
	}
}
